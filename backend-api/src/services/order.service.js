...
async function getManyOrders(query) {
  try {
    const { order_id, customer_id, order_date, order_payment_method, order_status, page = 1, limit = 5 } = query;
    ...
    let results = await orderRepository()
      .where((builder) => {
        ...
        if (order_payment_method) {
          builder.where('order_payment_method', 'like', `%${order_payment_method}%`);
        }
        if (order_status) {
